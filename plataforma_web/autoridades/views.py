...
@router.get("", response_model=LimitOffsetPage[schemas.AutoridadOut])
...
async def consultar_una_autoridad_con_clave(
    clave: str,
    current_user: UsuarioEnBD = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
    """Obtener una Autoridad a partir de su clave"""
    if not current_user.permissions & Permiso.VER_CATALOGOS == Permiso.VER_CATALOGOS:
        raise HTTPException(status_code=403, detail="Forbidden (no tiene permiso).")
    try:
...
async def consultar_una_autoridad(
    autoridad_id: int,
    current_user: UsuarioEnBD = Depends(get_current_active_user),
    db: Session = Depends(get_db),
):
    """Obtener una Autoridad a partir de su id"""
    if not current_user.permissions & Permiso.VER_CATALOGOS == Permiso.VER_CATALOGOS:
        raise HTTPException(status_code=403, detail="Forbidden (no tiene permiso).")
